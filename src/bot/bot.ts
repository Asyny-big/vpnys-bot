...
export function buildBot(deps: BotDeps): Bot {
  ...
  const showGuide = async (ctx: any): Promise<void> => {
    const text = [
      ...
      "",
      "🌍 <b>Серверы «Обход» — для мобильного интернета</b>",
      "<blockquote>Если оператор режет мобильный интернет и работают только <b>VK, Яндекс, Max, Госуслуги</b> — обычные серверы могут не пускать.",
      "",
      "В таких сетях выбирай в приложении сервер с названием <b>«🌍 Обход №1»</b>, <b>«🌍 Обход №2»</b> и т.д.",
      "Они специально настроены, чтобы пробиваться сквозь «белый список» оператора.",
      "</blockquote>",
      "<i>💡 Не работает один — попробуй следующий по номеру.</i>",
      "",
      "📶 <b>Если подписка не добавляется на мобильном</b>",
      "<blockquote>На «белом списке» оператора <b>сама подписка может не подгружаться</b> — это ограничение сети, не VPN.",
      "",
      "Что делать:",
      "• Добавь подписку <b>по Wi-Fi</b> один раз",
      "• На мобильном просто переключайся на <b>«🌍 Обход»</b>",
      "</blockquote>",
      "",
